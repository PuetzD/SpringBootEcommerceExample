import {type DataProvider, type GetListParams, HttpError} from 'react-admin'
...
const resourcePaths: Record<CatalogResource, string> = {
    ...
    orderConfirmationStatus: '/api/admin/orders'
}
...
export const dataProvider = {
    ...
    async getOne(resource, params) {
        ...
        if (resource === 'orderConfirmationStatus') {
            const orderNumber = String(params.id)
            const response = await runWithReactAdminError(() =>
                ApiClient.get<OrderConfirmationStatus>(`${path}/${orderNumber}/confirmation-delivery`),
            )
            return {data: normalizeRecord<OrderConfirmationStatusRecord>({id: orderNumber, ...response})}
        }
    ...
    async getMany(resource, params) {
        if (resource === 'orders' || resource === 'customers' || resource === 'orderConfirmationStatus') {
